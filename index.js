...
app.get('/', async (req, res) => {
  const url = 'https://api.hubapi.com/crm/v3/objects/2-142901100?properties=name,description,type';
  const headers = {
  ...
  try {
    ...
    res.render('homepage', { title: 'Custom Object Table | IWH Practicum', records });
  } catch (error) {
    console.error(error);
...
app.post('/update-cobj', async (req, res) => {
  ...
  const url = 'https://api.hubapi.com/crm/v3/objects/2-142901100';
  const headers = {
    Authorization: `Bearer ${PRIVATE_APP_ACCESS}`,
    'Content-Type': 'application/json'
  };
